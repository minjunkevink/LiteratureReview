# Relvant Literature Review

### RL (for lab)

- [Foundation Policies with Hilbert Representations](https://arxiv.org/pdf/2402.15567)
- [METRA: Scalable Unsupervised RL with Metric-Aware Abstraction](https://arxiv.org/pdf/2310.08887)
- https://arxiv.org/pdf/2402.17135

### RL (personal research/exploration)

- [RL-VLM-F: Reinforcement Learning from Vision Language Foundation Model Feedback](https://arxiv.org/pdf/2402.03681)
- [RoboCLIP: One Demonstration is Enough to Learn Robot Policies](https://arxiv.org/pdf/2310.07899)
- [Hierarchical Reinforcement Learning: A Comprehensive Survey](https://dl.acm.org/doi/abs/10.1145/3453160)
- [CURL: Contrastive Unsupervised Representations for Reinforcement Learning](https://arxiv.org/pdf/2004.04136)

### Meta-Learning

- [Bayesian Model-Agnostic Meta-Learning](https://arxiv.org/pdf/1806.03836)
- [Human irrationality: both bad and good for reward inference](https://arxiv.org/pdf/2111.06956)

### Unfinished Papers

- [Open Problems and Fundamental Limitations of Reinforcement Learning from Human Feedback](https://arxiv.org/pdf/2307.15217)
- [Asking Easy Questions: A User-Friendly Approach to Active Reward Learning](https://arxiv.org/pdf/1910.04365)

### NLP

- [QLORA: Efficient Finetuning of Quantized LLMs](https://arxiv.org/pdf/2305.14314)
- [VeRA: Vector-based Random Matrix Adaptation](https://arxiv.org/pdf/2310.11454)
- [Neural Architecture Search with Bayesian Optimisation and Optimal Transport](https://proceedings.neurips.cc/paper_files/paper/2018/file/f33ba15effa5c10e873bf3842afb46a6-Paper.pdf)
